...
async function handleListings(req, res) {
  ...
  let query = supabase
    ...
    .select(`
      ...
      market_id,
      items!inner(id, type, rarity, name, emoji, stat_value, attack, crit_chance, defense, upgrade_level, block_chance),
...
async function handleBuy(req, res) {
  ...
  if (gameState.loaded) {
    ...
    if (gi) { gi.owner_id = buyer.id; gameState.markDirty('items', gi.id); }
  }
...
async function handleAttackCourier(req, res) {
  ...
  if (killed) {
    ...
    if (gameState.loaded) {
      ...
      if (drop) gameState.upsertDrop({ ...drop, courier_id: courier.id, item_id: courier.item_id, listing_id: courier.listing_id, lat: courier.current_lat, lng: courier.current_lng, drop_type: 'loot', picked_up: false });
    }

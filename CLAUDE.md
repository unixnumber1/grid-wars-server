...
- Стоимость: 25💎, 10 уровней, апгрейд за алмазы [25→100]
- Продажа: 50% от суммарных вложенных алмазов (постройка + апгрейды), `COLLECTOR_SELL_REFUND_PCT`
- Радиус: 200м, автосбор с шахт
- HP: 3K→90K по уровням
- Макс уровень шахт для сбора: lv×20 (lv1=20, lv10=200)
- Интервал сбора (`COLLECTOR_INTERVAL_MS`): lv1=20мин → lv10=2мин (тик каждые 60с, гейт по уровню)
- Per-mine clock: использует `mine.last_collected` (как ручной сбор), без двойного сбора и потери времени
- Бонусы автосбора идентичны ручному: фермерские income/capacity, клановая зона+буст, landlord (только пока владелец онлайн)
- Комиссия доставки: 0% (отключена)

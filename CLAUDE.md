...
- **Backend (fallback)**: Vercel Functions (`/api/*`) — те же эндпоинты, запасной вариант
- **БД**: Supabase (service-role key, RLS off)
- **Хостинг**: VPS overthrow.ru (93.123.30.179), PM2, Nginx, SSL на порту 8443
- **Telegram**: Bot API + WebApp SDK
- **URL frontend**: https://overthrow.ru:8443 (основной), https://grid-wars-two.vercel.app (fallback)
- **URL backend**: https://overthrow.ru:8443
- **Репо frontend**: https://github.com/unixnumber1/grid-wars-front.git
- **Репо backend**: https://github.com/unixnumber1/grid-wars-server.git
- **Автодеплой фронта**: scripts/front-watcher.js — polling GitHub API каждые 30с, при новом коммите копирует public/* на VPS

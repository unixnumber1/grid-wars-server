...
/**
 * Base cost curve (smooth, no milestone spikes).
 * Used by both getMineUpgradeCost and getMineIncome.
 */
...
/** Payback period in seconds at a given level (smooth curve). */
function getPayback(l) {
  if (l <= 50)  return 50 * Math.pow(432, (l - 1) / 49);
...
/** Real coins per second at `level`. Level 0 = inactive = 0. */
export function getMineIncome(level) {
  if (level === 0) return 0;
  // Use smooth base cost (without milestone x4) so income never drops
...
/** Fixed base player stats — no level scaling. */
export const BASE_PLAYER_ATTACK = 10;
export const BASE_PLAYER_HP     = 100;

/** Max HP for a player (fixed base, ignores level). */
export function getMaxHp(_level) {
  return BASE_PLAYER_HP;
}

/** Base attack damage for a player (fixed base, ignores level). */
...
/** HP of a mine at the given level. Doubles every 10 levels (tier change). */
export function getMineHp(level) {
  if (level <= 0) return 0;
  const tier = Math.floor((level - 1) / 10);
  const posInTier = (level - 1) % 10;
  const tierBase = 100 * Math.pow(2, tier);
  return Math.round(tierBase * Math.pow(1.08, posInTier));
}

/** Mine HP regen per hour (~25% of max_hp). */
export function getMineHpRegen(level) {
  if (level <= 0) return 0;
  return Math.max(1, Math.floor(getMineHp(level) * 0.25));
}

/** Apply mine HP regen based on elapsed time. */

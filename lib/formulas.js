// ─── Storage scale ────────────────────────────────────────────────────────────
// All monetary values in DB are stored as real_value / COIN_SCALE to avoid
// INTEGER overflow. Use toStored/toReal at every DB boundary.

export const COIN_SCALE = 1_000_000;
export const toStored   = (real)   => real / COIN_SCALE;
export const toReal     = (stored) => (stored ?? 0) * COIN_SCALE;
 ...
 * Five exponential tiers with a tier multiplier that jumps every 10 levels.
 */
export function getMineUpgradeCost(level) {
  let base;
  if      (level < 50)  base = 80 * Math.pow(1.065, level);
  else if (level < 100) base = 2_000_000 * Math.pow(1.085, level - 50);
  else if (level < 150) base = 60_000_000_000 * Math.pow(1.105, level - 100);
  else if (level < 200) base = 40_000_000_000_000 * Math.pow(1.13, level - 150);
  else                  base = 3_000_000_000_000_000 * Math.pow(1.18, level - 200);
  const tierMultiplier = 1 + 2.5 * Math.floor(level / 10);
...
export function getMineIncome(level) {
  if (level === 0) return 0;
  return Math.round(30 * Math.pow(level, 1.18));
}

/** Max real coins a mine can hold at `level` (~6 days of income). */
export function getMineCapacity(level) {
  if (level === 0) return 0;
  return getMineIncome(level) * 144;
}

/**
 * Accumulated real coins since last_collected ISO string,
 * capped at ~6-day capacity.
 */
export function calcAccumulatedCoins(level, lastCollectedISO) {
  if (level === 0) return 0;
  const elapsedHours = (Date.now() - new Date(lastCollectedISO).getTime()) / 3_600_000;
  const raw = getMineIncome(level) * Math.max(0, elapsedHours);

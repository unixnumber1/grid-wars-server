...
export async function getCityBounds(cityKey) {
  ...
  if (playersInCity && playersInCity.size > 0) {
    ...
    if (hasCoords) {
      const PAD = 0.018; // ~2km padding (was 5km — reduced to avoid fields/forests)
      const centerLat = (minLat + maxLat) / 2;
      const centerLng = (minLng + maxLng) / 2;

      const spreadKm = haversine(minLat, minLng, maxLat, maxLng) / 1000;
      // For large cities: collect per-player sub-zones instead of one giant bbox
      if (spreadKm > 20) {
        console.log(`[GEOCITY] ${cityKey}: player spread ${spreadKm.toFixed(1)}km > 20km, using player sub-zones`);
        const subZones = [];
        for (const tgId of playersInCity) {
          const pp = playerCityCache.get(tgId);
          if (!pp?.lat || !pp?.lng) continue;
          // Check if this player is already covered by an existing sub-zone
          const alreadyCovered = subZones.some(z =>
            haversine(pp.lat, pp.lng, (z[0]+z[1])/2, (z[2]+z[3])/2) < 4000
          );
          if (alreadyCovered) continue;
          subZones.push([pp.lat - PAD, pp.lat + PAD, pp.lng - PAD, pp.lng + PAD]);
        }
        const result = {
          lat: centerLat, lng: centerLng,
          boundingbox: [minLat - PAD, maxLat + PAD, minLng - PAD, maxLng + PAD],
          subZones,
          updatedAt: Date.now(),
        };
  ...
  // Fallback: Nominatim search by city name
  try {

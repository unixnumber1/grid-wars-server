...
// Load persistent cache on startup (sync is OK here — runs once before server starts)
try {
...
function persistCache() {
  if (_persistPending) return; // debounce
  ...
  setTimeout(() => {
    ...
    writeFile(CACHE_FILE, JSON.stringify(data)).catch(e => console.error('[geocity] error:', e.message));
  }, 5000);
}

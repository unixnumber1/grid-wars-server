...
// Uses player coordinates as viewbox hint to disambiguate city names
export async function getCityBounds(cityKey) {
  ...
  try {
    ...
    // Find a player in this city to use as location hint
    let viewboxParam = '';
    const playersInCity = cityPlayersCache.get(cityKey);
    if (playersInCity && playersInCity.size > 0) {
      const firstTgId = [...playersInCity][0];
      const playerData = playerCityCache.get(firstTgId);
      if (playerData?.lat && playerData?.lng) {
        // ~50km viewbox around the player
        const pad = 0.5;
        viewboxParam = `&viewbox=${playerData.lng - pad},${playerData.lat + pad},${playerData.lng + pad},${playerData.lat - pad}&bounded=1`;
      }
    }

    const url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(q)}&format=json&limit=1&accept-language=ru${viewboxParam}`;
    const res = await fetch(url, {
      headers: { 'User-Agent': 'Overthrow Game/1.0' },
      signal: AbortSignal.timeout(10000),
    });
    ...
    if (!data.length && viewboxParam) {
      const fallbackUrl = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(q)}&format=json&limit=1&accept-language=ru`;

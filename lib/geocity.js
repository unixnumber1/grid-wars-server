...
export async function getCityBounds(cityKey) {
  ...
  if (playersInCity && playersInCity.size > 0) {
    ...
    if (hasCoords) {
      ...
      // If player spread > 20km, bbox is too large — fall through to Nominatim
      const spreadKm = haversine(minLat, minLng, maxLat, maxLng) / 1000;
      if (spreadKm > 20) {
        console.log(`[GEOCITY] ${cityKey}: player spread ${spreadKm.toFixed(1)}km > 20km, using Nominatim bounds`);
      } else {
        const result = {
          lat: centerLat,
          lng: centerLng,

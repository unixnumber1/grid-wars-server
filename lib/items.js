...
export function getUpgradedStats(item) {
  ...
  if (item.type === 'sword') {
    ...
    result.crit_chance = (item.base_crit_chance || item.crit_chance || 0) + lvl * 0.17;
  ...
  if (item.type === 'axe') {
    result.attack = Math.floor((item.base_attack || item.attack) * mul);
    if (item.rarity === 'mythic') result.execution_chance = 7 + (lvl / 90) * 10;
    else if (item.rarity === 'legendary') result.execution_chance = 13 + (lvl / 100) * 7;
  ...
  if (item.type === 'shield') {
    ...
    if ((item.rarity === 'mythic' || item.rarity === 'legendary') && item.block_chance) {
      ...
      if (item.rarity === 'mythic') result.block_chance = baseBlock + (lvl / 90) * 5;
      else result.block_chance = baseBlock + (lvl / 100) * 6;
    }
  }
  return result;
}
